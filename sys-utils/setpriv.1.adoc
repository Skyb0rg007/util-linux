...
+
The syntax is *$category*[**:**__$right__[**,**__...__]]. Without any rights the whole
category is restricted. The available categories are *fs* (filesystem accesses,
for example _read-file_ or _make-dir_), *net* (TCP and UDP port accesses, that is
_bind-tcp_, _connect-tcp_, _bind-udp_ and _connect-send-udp_) and *scoped* (IPC
accesses that reach outside of the sandbox, that is _abstract-unix-socket_ and
_signal_). Unlike the other categories, *scoped* cannot be relaxed by
*--landlock-rule*.
+
...
+
Block outgoing TCP connections and signals to processes outside the sandbox:
+
*setpriv --landlock-access net:connect-tcp --landlock-access scoped:signal*
+
An access right that the running kernel does not support is rejected; use
*--landlock-support* to inspect what is available. For a complete set of
supported access categories use *setpriv --help*.
+
The set of access categories is subject to change and may be extended in future
versions to support new access types.
+
Appending *-quiet* to a category name (*fs-quiet*, *net-quiet* or
*scoped-quiet*) both restricts and silences Landlock's audit logging for the
given rights in one step, equivalent to using the plain category name and
then separately marking the same rights quiet. Rights restricted through the
plain category name keep being logged normally. Combine both forms across
multiple *--landlock-access* options to restrict a wider set of rights while
only silencing a subset of them, for example:
+
*setpriv --landlock-access fs:read-file,write-file --landlock-access fs-quiet:write-file*
+
For *fs* and *net*, a silenced right is only actually silenced for denials
attributed to a rule marked *quiet* (see *--landlock-rule*); other denials of
the same right are logged as usual. For *scoped*, no per-rule marking is
needed. *-quiet* categories require the running kernel to support Landlock
quiet logging, available since Landlock ABI 10; use *--landlock-support* to
check the running kernel's ABI version.
...
+
And grant the right to connect to port 443:
+
*--landlock-rule net-port:connect-tcp:443*
+
The available rule types are *path-beneath*, whose _$rulearg_ is a path and
which relaxes the *fs* access category, and *net-port*, whose _$rulearg_ is a
TCP or UDP port number and which relaxes the *net* access category. A rule
requires its access category to be restricted by *--landlock-access*.
+
Appending *-quiet* to _$ruletype_ (*path-beneath-quiet*, *net-port-quiet*)
marks the rule as one whose denials can be silenced, for the rights
restricted through a *-quiet* *--landlock-access* category (see
*--landlock-access*). This requires the running kernel to support Landlock
quiet logging.

*--landlock-restrict-self* _flags_::

Configure the logging behavior of the sandbox created by *--landlock-access* and
*--landlock-rule*. The argument is a comma-separated list of flags. The available
flags are *log-same-exec-off* (stop logging denied accesses from this thread until
its next *execve*(2)), *log-new-exec-on* (log denied accesses from this domain after
an *execve*(2)) and *log-subdomains-off* (stop logging denied accesses from nested
landlock domains created within the sandbox). Requires at least one access category
to be restricted by *--landlock-access*.
+
A flag that the running kernel does not support is rejected; use
*--list-landlock-restrict-self-flags* to inspect what is available.
...
List the landlock access categories that are usable with *--landlock-access* and
*--landlock-rule* on the running kernel.

*--list-landlock-rights* _access_::
List the rights of the given landlock access category that are supported by the running
kernel.

*--list-landlock-restrict-self-flags*::
List the *--landlock-restrict-self* flags that are supported by the running kernel.
