...
for example _read-file_ or _make-dir_), *net* (TCP port accesses, that is
_bind-tcp_ and _connect-tcp_) and *scoped* (IPC accesses that reach outside of
the sandbox, that is _abstract-unix-socket_ and _signal_). Unlike the other
categories, *scoped* cannot be relaxed by *--landlock-rule*.
+
...
TCP port number and which relaxes the *net* access category. A rule requires
its access category to be restricted by *--landlock-access*.
